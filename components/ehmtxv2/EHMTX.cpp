...
namespace esphome::ehmtx
{
  ...
  EHMTX::EHMTX() : PollingComponent(POLLINGINTERVAL)
  {
    ...
    this->top_indicator_color_inactive = Color(0, 0, 200);
    this->top_indicator_color_active = Color(255, 100, 100);
    this->top_indicator_mask = 0; 
 ...
 */
  void EHMTX::show_alarm(int32_t r, int32_t g, int32_t b, int32_t size)
  {
    ...
    if (size > 0)
    {
      ...
      this->display_indicator = this->IntToBits(this->display_indicator, size, 3, 0);
      ESP_LOGD(TAG, "Show alarm (top right) size: %d color r: %d g: %d b: %d", size, r, g, b);
    }
    else
    {
      this->hide_alarm();
 ...
 */
  ...
  void EHMTX::draw_gauge()
  {
    if (this->display_gauge)
    {
      ...
      {
        this->display->draw_pixel_at(63, y, this->cgauge[y]);
      }
      this->display->line(62, 7, 62, 0, esphome::display::COLOR_OFF);
    }
  }
  ...
  void EHMTX::setup()
  {
    ...
    static const std::array<std::string, 1> arg_time = {"time"};
    register_service(&EHMTX::hold_screen, "hold_screen", arg_time);

    static const std::array<std::string, 4> arg_rgb_size = {"r", "g", "b", "size_max_3"};
    register_service(&EHMTX::show_alarm, "show_alarm", arg_rgb_size);

    #ifdef EHMTXv2_RBINDICATOR
    register_service(&EHMTX::show_rindicator, "show_rindicator", arg_rgb_size);
    register_service(&EHMTX::hide_rindicator, "hide_rindicator");
    #endif

    #ifdef EHMTXv2_GAUGE
    static const std::array<std::string, 7> arg_gauge = {"percent", "r", "g", "b", "bg_r", "bg_g", "bg_b"};
    register_service(&EHMTX::show_gauge, "show_gauge", arg_gauge);
    register_service(&EHMTX::hide_gauge, "hide_gauge");
    #endif

    static const std::array<std::string, 3> arg_rgb = {"r", "g", "b"};
    register_service(&EHMTX::set_today_color, "set_today_color", arg_rgb);
    register_service(&EHMTX::set_weekday_color, "set_weekday_color", arg_rgb);
    register_service(&EHMTX::set_clock_color, "set_clock_color", arg_rgb);
    register_service(&EHMTX::set_text_color, "set_text_color", arg_rgb);
    register_service(&EHMTX::set_solid_color, "set_solid_color", arg_rgb);
    register_service(&EHMTX::set_calendar_color, "set_calendar_color", arg_rgb);

    static const std::array<std::string, 8> arg_infotext = {"left_r", "left_g", "left_b", "right_r", "right_g", "right_b", "default_font", "y_offset"};
    register_service(&EHMTX::set_infotext_color, "set_infotext_color", arg_infotext);

    static const std::array<std::string, 1> arg_mode = {"mode"};
    register_service(&EHMTX::expand_icon_to_9, "expand_icon_to_9", arg_mode);

    static const std::array<std::string, 2> arg_icon_mode = {"icon_name", "mode"};
    register_service(&EHMTX::del_screen, "del_screen", arg_icon_mode);
    register_service(&EHMTX::force_screen, "force_screen", arg_icon_mode);

    static const std::array<std::string, 3> arg_icon_life_time = {"icon_name", "lifetime", "screen_time"};
    register_service(&EHMTX::full_screen, "full_screen", arg_icon_life_time);

    static const std::array<std::string, 8> arg_icon_text_life_time_font_rgb = {"icon_name", "text", "lifetime", "screen_time", "default_font", "r", "g", "b"};
    register_service(&EHMTX::icon_screen, "icon_screen", arg_icon_text_life_time_font_rgb);
    register_service(&EHMTX::icon_text_screen, "icon_text_screen", arg_icon_text_life_time_font_rgb);

    static const std::array<std::string, 7> arg_icon_text_time_font_rgb = {"icon_name", "text", "screen_time", "default_font", "r", "g", "b"};
    register_service(&EHMTX::alert_screen, "alert_screen", arg_icon_text_time_font_rgb);
    register_service(&EHMTX::flash_alert_screen, "flash_alert_screen", arg_icon_text_time_font_rgb);

    static const std::array<std::string, 4> arg_icon_text_time_font = {"icon_name", "text", "screen_time", "default_font"};
    register_service(&EHMTX::rainbow_alert_screen, "rainbow_alert_screen", arg_icon_text_time_font);

    static const std::array<std::string, 7> arg_icon_life_time_font_rgb = {"icon_name", "lifetime", "screen_time", "default_font", "r", "g", "b"};
    register_service(&EHMTX::icon_clock, "icon_clock", arg_icon_life_time_font_rgb);
    register_service(&EHMTX::icon_date, "icon_date", arg_icon_life_time_font_rgb);
    register_service(&EHMTX::icon_date_time_screen, "icon_date_time_screen", arg_icon_life_time_font_rgb);

    static const std::array<std::string, 5> arg_icon_text_life_time_font = {"icon_name", "text", "lifetime", "screen_time", "default_font"};
    register_service(&EHMTX::rainbow_icon_screen, "rainbow_icon_screen", arg_icon_text_life_time_font);
    register_service(&EHMTX::rainbow_icon_text_screen, "rainbow_icon_text_screen", arg_icon_text_life_time_font);

    static const std::array<std::string, 7> arg_text_life_time_font_rgb = {"text", "lifetime", "screen_time", "default_font", "r", "g", "b"};
    register_service(&EHMTX::text_screen, "text_screen", arg_text_life_time_font_rgb);

    static const std::array<std::string, 4> arg_text_life_time_font = {"text", "lifetime", "screen_time", "default_font"};
    register_service(&EHMTX::rainbow_text_screen, "rainbow_text_screen", arg_text_life_time_font);

    static const std::array<std::string, 2> arg_life_time = {"lifetime", "screen_time"};
    register_service(&EHMTX::blank_screen, "blank_screen", arg_life_time);

    static const std::array<std::string, 5> arg_life_time_rgb = {"lifetime", "screen_time", "r", "g", "b"};
    register_service(&EHMTX::color_screen, "color_screen", arg_life_time_rgb);

    static const std::array<std::string, 1> arg_value = {"value"};
    register_service(&EHMTX::set_brightness, "brightness", arg_value);

#ifdef EHMTXv2_ADV_CLOCK
    register_service(&EHMTX::set_clock_infotext_color, "set_clock_infotext_color", arg_infotext);
    register_service(&EHMTX::set_date_infotext_color, "set_date_infotext_color", arg_infotext);
    static const std::array<std::string, 9> arg_adv_clock = {"hr", "hg", "hb", "mr", "mg", "mb", "sr", "sg", "sb"};
    register_service(&EHMTX::set_adv_clock_color, "set_adv_clock_color", arg_adv_clock);
#endif

    static const std::array<std::string, 10> arg_text_prog = {"text", "value_max_4_characters", "progress", "lifetime", "screen_time", "default_font", "value_color_as_progress", "r", "g", "b"};
    register_service(&EHMTX::text_screen_progress, "text_screen_progress", arg_text_prog);

    static const std::array<std::string, 9> arg_icon_prog = {"icon_name", "text", "progress", "lifetime", "screen_time", "default_font", "r", "g", "b"};
    register_service(&EHMTX::icon_screen_progress, "icon_screen_progress", arg_icon_prog);

    static const std::array<std::string, 8> arg_prog_color = {"icon_name", "mode", "r", "g", "b", "bg_r", "bg_g", "bg_b"};
    register_service(&EHMTX::set_progressbar_color, "set_progressbar_color", arg_prog_color);

    static const std::array<std::string, 6> arg_icon_prog_scr = {"icon_name", "text", "prognosis", "lifetime", "screen_time", "default_font"};
    register_service(&EHMTX::icon_prognosis_screen, "icon_prognosis_screen", arg_icon_prog_scr);

    static const std::array<std::string, 9> arg_icon_prog_scr_rgb = {"icon_name", "text", "prognosis", "lifetime", "screen_time", "default_font", "r", "g", "b"};
    register_service(&EHMTX::icon_prognosis_screen_rgb, "icon_prognosis_screen_rgb", arg_icon_prog_scr_rgb);
#endif

    static const std::array<std::string, 2> arg_top_ind = {"position_between_1_and_6", "state"};
    register_service(&EHMTX::set_top_indicator, "set_top_indicator", arg_top_ind);

    static const std::array<std::string, 6> arg_top_ind_col = {"r", "g", "b", "bg_r", "bg_g", "bg_b"};
    register_service(&EHMTX::set_top_indicator_def_colors, "set_top_indicator_colors", arg_top_ind_col);
  ...
  void EHMTX::set_clock_color(int32_t r, int32_t g, int32_t b)
  {
    ...
    {
      if (this->queue[i]->mode == MODE_ICON_CLOCK ||
...
uint8_t EHMTX::find_oldest_queue_element()
  {
    ...
    for (size_t i = 0; i < MAXQUEUE; i++)
    {
      if (this->queue[i]->mode != MODE_EMPTY)
      {
        ...
        if (this->queue[i]->last_time < oldest)
        {
          oldest = this->queue[i]->last_time;
          hit = i;
        }
      }
    }
    ...
    {
      // Visszaesési logika: ha egyetlen aktív slot sem ment át a fenti időellenőrzésen
      for (size_t i = 0; i < MAXQUEUE; i++)
      {
        if (this->queue[i]->mode == MODE_EMPTY) continue;
        if (this->night_mode)
        {
          bool skip = true;
          for (auto id : EHMTXv2_CONF_NIGHT_MODE_SCREENS)
  ...
  uint8_t EHMTX::find_last_clock()
  {
    ...
    if (EHMTXv2_CLOCK_INTERVAL > 0)
    {
      ...
      for (size_t i = 0; i < MAXQUEUE; i++)
      {
        ...
        if ((this->queue[i]->mode == MODE_ICON_DATE_TIME) || (this->queue[i]->mode == MODE_ICON_CLOCK))
        {
...
//      this->icons[this->queue[this->screen_pointer]->icon]->set_frame(0);
    ...
    // A ciklus magába foglalja a teljes logikát
    for (auto *t : on_next_screen_triggers_)
    {
        // A t->process() paraméter nélküli hívást kivettük, mert nem illik ide
        ...
        // A trigger hívása a cikluson BELÜL történik
  ...
  void EHMTX::tick()
  {
    ...
    {
      // H - Piros
      this->display->print(3, 1, this->default_font, Color(255, 0, 0), "H");
      // O - Narancs
      this->display->print(9, 1, this->default_font, Color(255, 165, 0), "O");
      // L - Sárga
      this->display->print(15, 1, this->default_font, Color(255, 255, 0), "L");
      // L - Zöld
      this->display->print(21, 1, this->default_font, Color(0, 255, 0), "L");
      // Y - Cián
      this->display->print(27, 1, this->default_font, Color(0, 255, 255), "Y");

      // 8 - Kék
      this->display->print(37, 1, this->default_font, Color(0, 0, 255), "8");
      // x - Lila
      this->display->print(43, 1, this->default_font, Color(128, 0, 128), "X");
      // 6 - Magenta
      this->display->print(49, 1, this->default_font, Color(255, 0, 255), "6");
      // 4 - Fehér
      this->display->print(55, 1, this->default_font, Color(255, 255, 255), "4");
    }
  }
  ...
  void EHMTX::flash_alert_screen(std::string iconname, std::string text, int32_t screen_time, bool default_font, int32_t r, int32_t g, int32_t b)
  {
    uint8_t icon = this->find_icon(iconname.c_str());
    ...
    EHMTX_queue *screen = this->find_mode_queue_element(MODE_FLASH_ALERT_SCREEN);

    screen->text = text;
    screen->text_color = Color(r, g, b); // Ez lesz az alap szín (pl. piros)
    screen->default_font = default_font;
    screen->mode = MODE_FLASH_ALERT_SCREEN;
    screen->icon_name = iconname;
    screen->icon = icon;
    screen->calc_scroll_time(text, screen_time);
    ...
    ESP_LOGD(TAG, "flash alert screen icon: %d iconname: %s text: %s screen_time: %d", icon, iconname.c_str(), text.c_str(), screen_time);
    screen->status();

    force_screen(iconname, MODE_FLASH_ALERT_SCREEN);
  }
  ...
  void EHMTX::icon_date_time_screen(std::string iconname, int32_t lifetime, int32_t screen_time, bool default_font, int32_t r, int32_t g, int32_t b)
  {
    std::string ic = get_icon_name(iconname);
    std::string id = get_screen_id(iconname);
    uint8_t icon = this->find_icon(ic.c_str());
    ...
    if (icon == MAXICONS)
    {
      ESP_LOGW(TAG, "icon %d/%s not found => default: 0", icon, ic.c_str());
      icon = 0;
      for (auto *t : on_icon_error_triggers_) { t->process(ic); }
    }
    
    // Keressük a sort (az új móddal)
    EHMTX_queue *screen = this->find_mode_icon_queue_element(MODE_ICON_DATE_TIME, id);

    screen->text_color = Color(r, g, b);
    screen->default_font = default_font;
    screen->mode = MODE_ICON_DATE_TIME;
    screen->icon_name = id;
    screen->icon = icon;
    ...
    ESP_LOGD(TAG, "icon date time screen icon: %d", icon);
    screen->status();
  }
...
#define max3(x, y, z) ((x) > (y) ? ((x) > (z) ? (x) : (z)) : ((y) > (z) ? (y) : (z)))
    ...
    if (this->show_day_of_week)
    {
      ...
      if (!small)
      {
        ...
        for (uint8_t i = 0; i <= 6; i++)
        {
          ...
          {
            this->display->line(start_x + i * 6, ypos + 7, start_x + i * 6 + 3, ypos + 7, this->today_color);
          }
          else
          {
            this->display->line(start_x + i * 6, ypos + 7, start_x + i * 6 + 3, ypos + 7, this->weekday_color);
            if (accent_color != esphome::display::COLOR_OFF)
            {
              this->display->line(start_x + i * 6 + 2, ypos + 7, start_x + i * 6 + 2, ypos + 7, accent_color);
            }
          }
        }
      }
      ...
      {
        // Itt is beállítunk egy kezdő eltolást (offset)
        // Mivel ez az ág akkor fut le, ha ikon van (small=true), 
        // a 12-es pixelről indítjuk, hogy az ikon (8-10 pixel) után kezdődjön.
        uint8_t start_x = 12; 

        for (uint8_t i = 0; i <= 6; i++)
        {
          // i * 6 biztosítja a 4 pixel vonal + 2 pixel szünet ritmust
          uint16_t x_start = start_x + (i * 6);
          uint16_t x_end = x_start + 3; // +3, hogy összesen 4 pixel széles legyen

          if (((!EHMTXv2_WEEK_START) && (dow == i)) ||
              ((EHMTXv2_WEEK_START) && ((dow == (i + 1)) || ((dow == 0 && i == 6)))))
          {
            // MAI NAP
            this->display->line(x_start, ypos + 7, x_end, ypos + 7, this->today_color);
          }
          else
          {
            // TÖBBI NAP
            this->display->line(x_start, ypos + 7, x_end, ypos + 7, this->weekday_color);
            
            if (accent_color != esphome::display::COLOR_OFF)
            {
              // Az accent (kiemelő) pixel a vonal 3. pixelére kerül
              this->display->line(x_start + 2, ypos + 7, x_start + 2, ypos + 7, accent_color);
            }
          }
        }
      }
  ...
  bool EHMTX::draw_date(std::string format, esphome::display::BaseFont *font, Color color, int32_t xpos, int32_t ypos)
  {
   ...
   void EHMTX::draw_text(const std::string &text, esphome::display::BaseFont *font, Color color, int32_t xpos, int32_t ypos)
  {
  #ifdef EHMTXv2_MULTICOLOR_TEXT
  ...
  void EHMTX::draw_rainbow_text(const std::string &text, esphome::display::BaseFont *font, int32_t xpos, int32_t ypos)
  {
  ...
  void EHMTX::draw_alarm()
  {
    ...
    // Minden X koordinátát csökkentettünk 2-vel (63->61, 62->60, 61->59)
    if (size > 2)
    {
      this->display->line(62, 2, 60, 0, this->alarm_color);
    }
    if (size > 1)
    {
      this->display->draw_pixel_at(61, 0, this->alarm_color);
      this->display->draw_pixel_at(62, 1, this->alarm_color);
    }
    if (size > 0)
    {
      this->display->draw_pixel_at(62, 0, this->alarm_color);
    }
  }
  ...
  void EHMTX::draw_rindicator()
  {
    ...
    // Minden X koordinátát csökkentettünk 2-vel
    if (size > 2)
    {
      this->display->line(62, 5, 60, 7, this->rbindicator_color);
    }

    if (size > 1)
    {
      this->display->draw_pixel_at(61, 7, this->rbindicator_color);
      this->display->draw_pixel_at(62, 6, this->rbindicator_color);
    }

    if (size > 0)
    {
      this->display->draw_pixel_at(62, 7, this->rbindicator_color);
    }
  }
#endif
...
void HOT EHMTX::draw()
  {
    ...
    {
      // 1. Kirajzoljuk magát a képernyőt (az aktuális tartalmat)
      if (this->screen_pointer != MAXQUEUE)
      {
        this->queue[this->screen_pointer]->draw();

        uint8_t current_mode = this->queue[this->screen_pointer]->mode;
        if (current_mode == MODE_ICON_CLOCK || 
            current_mode == MODE_ICON_DATE || 
            current_mode == MODE_ICON_DATE_TIME) 
        {
            this->draw_top_indicators();
        }        

      }

      // 2. Eldöntjük, kellenek-e az extra rétegek (indikátorok, gauge)
      // akkor NEM kellenek, ha teljes képernyős módban vagyunk
      bool show_overlays = (this->queue[this->screen_pointer]->mode != MODE_TEXT_PROGRESS &&
                            this->queue[this->screen_pointer]->mode != MODE_ICON_PROGRESS &&
                            this->queue[this->screen_pointer]->mode != MODE_FULL_SCREEN &&
                            this->queue[this->screen_pointer]->mode != MODE_COLOR &&
                            this->queue[this->screen_pointer]->mode != MODE_BLANK &&
                            this->queue[this->screen_pointer]->mode != MODE_ALERT_SCREEN &&
                            this->queue[this->screen_pointer]->mode != MODE_FLASH_ALERT_SCREEN &&
                            this->queue[this->screen_pointer]->mode != MODE_RAINBOW_ALERT_SCREEN);

      // Ha kellenek a rétegek, akkor kirajzoljuk őket sorban:
  ...
  void EHMTX::set_top_indicator(int32_t position, bool state)
  {
    if (position < 1 || position > 6) {
        ESP_LOGW(TAG, "ERROR: Top indicator position can only be between 1-6! (Now is: %d)", (int)position);
        return;
    }

    if (state) {
        this->top_indicator_mask |= (1 << (position - 1));
    } else {
        this->top_indicator_mask &= ~(1 << (position - 1));
    }
    ESP_LOGD(TAG, "Top indicator %d set to %s", position, state ? "ON" : "OFF");
  }

  void EHMTX::set_top_indicator_def_colors(int32_t r, int32_t g, int32_t b, int32_t bg_r, int32_t bg_g, int32_t bg_b)
  {
    this->top_indicator_color_active = Color((uint8_t)r, (uint8_t)g, (uint8_t)b);
    this->top_indicator_color_inactive = Color((uint8_t)bg_r, (uint8_t)bg_g, (uint8_t)bg_b);
    ESP_LOGD(TAG, "Top indicator colors updated");
  }

  void EHMTX::set_show_top_indicators(bool b)
  {
    this->show_top_indicators = b;
    ESP_LOGI(TAG, "%sshow top indicators", b ? ESPHOME_F("") : ESPHOME_F("don't "));
  }  

  void EHMTX::draw_top_indicators()
  {
    if (!this->show_top_indicators) return;
    // 6 indikátor. Kezdjük a 12. pixeltől, hogy az ikon ne takarja ki bal oldalt.
    int start_x = 12; // Eltolás az ikon miatt
    int width = 4;    // Egy indikátor szélessége
    int gap = 3;      // Szünet az indikátorok között
    int y_pos = 0;    // Legfelső sor

    for (int i = 0; i < 6; i++) {
        int x = start_x + (i * (width + gap));
        
        Color c;
        // Megnézzük, hogy az i-edik bit 1-e
        if ((this->top_indicator_mask >> i) & 1) {
            c = this->top_indicator_color_active;
        } else {
            c = this->top_indicator_color_inactive;
        }

        this->display->line(x, y_pos, x + width - 1, y_pos, c);
    }
  }
