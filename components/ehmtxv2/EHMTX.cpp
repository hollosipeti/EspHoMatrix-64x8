 ...
 */
  ...
  void EHMTX::setup()
  {
    ...
    #endif
    #ifdef EHMTXv2_LTINDICATOR
    register_service(&EHMTX::show_ltindicator, "show_ltindicator", {"r", "g", "b", "size_max_3"});
    register_service(&EHMTX::hide_ltindicator, "hide_ltindicator");
    #endif
    #ifdef EHMTXv2_LBINDICATOR
    register_service(&EHMTX::show_lindicator, "show_lindicator", {"r", "g", "b", "size_max_3"});
  ...
  uint8_t EHMTX::find_last_clock()
  {
    ...
    if (EHMTXv2_CLOCK_INTERVAL > 0)
    {
      ...
      for (size_t i = 0; i < MAXQUEUE; i++)
      {
        ...
        if ((this->queue[i]->mode == MODE_CLOCK) || (this->queue[i]->mode == MODE_ICON_DATE_TIME) || (this->queue[i]->mode == MODE_ICON_CLOCK))
        {
    ...
    if ((this->is_running) && (this->show_display))
    {
          ...
          this->queue[this->screen_pointer]->mode != MODE_FULL_SCREEN)
      {
        ...
        if (this->queue[this->screen_pointer]->mode != MODE_ICON_SCREEN &&
            ...
            this->queue[this->screen_pointer]->mode != MODE_ICON_DATE_TIME
            )
        {
#endif
        #ifdef EHMTXv2_LTINDICATOR
