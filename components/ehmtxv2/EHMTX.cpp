 ...
 */
  ...
  void EHMTX::set_boot_logo(std::string logo)
  {
    ...
    #else
      this->boot_logo = new uint8_t[512];
    #endif
    }
  ...
  void EHMTX::tick()
  {
    ...
    else
    {
      ...
      if (this->boot_logo != NULL)
      {
        ...
        for (uint8_t x = 0; x < 64; x++)
        {
          for (uint8_t y = 0; y < 8; y++)
          {
            ...
            this->display->draw_pixel_at(x, y, this->boot_logo[x + y * 64]);
      #else
        #if defined EHMTXv2_ADV_BOOT_MODE_2 || defined EHMTXv2_ADV_BOOT_MODE_4
            if (this->boot_logo[x + y * 64] == 1)
            {
        ...
        #endif
        #if defined EHMTXv2_ADV_BOOT_MODE_6 || defined EHMTXv2_ADV_BOOT_MODE_7
            if (CHECK_BIT(this->boot_logo[x + y * 64], ((this->boot_anim % 64) / 8)))
            {
        ...
        if (this->boot_anim % 8 == 0 )
        {
          uint8_t w = (2 + (uint8_t)(64 / 16) * (this->boot_anim * 2 / 16)) % 64;
          uint8_t l = 16 - w / 2;
          uint8_t r = 15 + w / 2;
          for (uint8_t y = 0; y < 8; y++)
          {
      #if defined EHMTXv2_ADV_BOOT_MODE_3 || defined EHMTXv2_ADV_BOOT_MODE_5
            if (this->boot_logo[l + y * 64] == 1)
            {
            ...
            this->display->draw_pixel_at(l, y, this->boot_logo[l + y * 64]);
            this->display->draw_pixel_at(r, y, this->boot_logo[r + y * 64]);
      #endif
          }
        }
    #endif
      ...
      {
        uint8_t w = 2 + ((uint8_t)(64 / 16) * (this->boot_anim / 16)) % 64;
        uint8_t l = 64 / 2 - w / 2;
