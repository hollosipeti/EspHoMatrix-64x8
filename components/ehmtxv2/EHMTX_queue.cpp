...
if (this->pixels_ < width)
  ...
  void EHMTX_queue::draw()
  {
    ...
    {
      // --- ÚJ RÉSZ KEZDTE ---
      // Ellenőrizzük, hogy aktív-e bármelyik jobb oldali jelző (Alarm: 0-2 bit, Jobb alsó: 7-9 bit)
      bool right_indicators_active = (this->config_->BitsToInt(this->config_->display_indicator, 0, 2) > 0) || 
                                     (this->config_->BitsToInt(this->config_->display_indicator, 7, 9) > 0);
      
      // Ha aktív, akkor 4 pixelt levonunk a szélességből, különben 0-t.
      uint8_t reduce_width = right_indicators_active ? 4 : 0;
      // --- ÚJ RÉSZ VÉGE ---
      ...
      case MODE_ICON_TEXT_SCREEN:
      case MODE_RAINBOW_ICON_TEXT_SCREEN:
        this->config_->display->start_clipping(0, 0, 64 - reduce_width, 8);
